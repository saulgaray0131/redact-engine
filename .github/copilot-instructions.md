...
## Database Migrations
- When entities or entity configurations change, create a migration: `dotnet ef migrations add <Name> --project RedactEngine.Infrastructure --startup-project RedactEngine.ApiService`.
- Migration files go in `RedactEngine.Infrastructure/Migrations/` and must be committed.
- Migrations are applied automatically on startup via `DatabaseMigrationRunner`. Do not run `dotnet ef database update` manually.
- To remove an uncommitted migration: `dotnet ef migrations remove --project RedactEngine.Infrastructure --startup-project RedactEngine.ApiService`.
...
## Messaging
- Azure Service Bus uses the **Basic** tier in production, which supports **queues only** (no topics or subscriptions).
- The Dapr pubsub component is configured as `pubsub.azure.servicebus.queues`. Do not switch to `pubsub.azure.servicebus`
- Dapr topic/queue names are defined in `RedactEngine.Shared/PubSub/`.
